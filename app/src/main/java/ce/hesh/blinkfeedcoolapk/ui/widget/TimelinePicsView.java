...
import ce.hesh.blinkfeedcoolapk.bean.PicSize;
import ce.hesh.blinkfeedcoolapk.bean.PicUrls;
import ce.hesh.blinkfeedcoolapk.ui.BizFragment;
import ce.hesh.blinkfeedcoolapk.util.TimelineThumbBitmapCompress;
     ...
     /*   if (ownerFragment instanceof ProfileTimelineFragment) {
        ...
        switch (size) {
            ...
            case 6:
                ...
                try {
                    random = (int) (Integer.valueOf(mStatusContent.getId()) % 3);
                } catch (Exception e) {
                }
                if (random == 0) {
                    height = imgH * 2 + gap;
    ...
    public void displayPics() {
        ...
        for (int i = 0; i < getChildCount(); i++) {
            ...
            if (i >= picRects.length) {
                ...
                //获取中图url
                String url = getStatusMulImage(picUrls[i].getMidpic());
                //如果是一个图片，直接下载原图
                if (size==1)
                    url = getStatusMulImage(picUrls[i].getThumbnail_pic());
...
//				}
    ...
    public void setPics(FeedInfo status, ABaseFragment ownerFragment) {
...
//        if (!replace) return;
        ...
        if (mStatusContent.getPicArr() !=null && mStatusContent.getPicArr().length !=0){
            PicUrls[] picUrls = new PicUrls[mStatusContent.getPicArr().length];
            for (int i = 0;i<mStatusContent.getPicArr().length;i++){
                PicUrls pic = new PicUrls();
                pic.setThumbnail_pic(mStatusContent.getPicArr()[i]);
                picUrls[i] = pic;
            }
